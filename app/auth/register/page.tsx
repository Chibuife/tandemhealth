...
const passwordStrength = (password: string): { label: string; ratio: number; color: string } => {
  if (!password) return { label: '', ratio: 0, color: 'bg-slate-200' };
  ...
  if (score <= 1) return { label: 'Weak', ratio: 0.25, color: 'bg-rose-400' };
  if (score <= 3) return { label: 'Medium', ratio: 0.6, color: 'bg-amber-400' };
    ...
    <div className="flex min-h-screen">
      ...
      <div className="relative hidden w-[420px] shrink-0 flex-col justify-between overflow-hidden bg-[#FBF6E3] px-10 py-10 lg:flex">
        <div>
          ...
          <h1 className="mb-4 text-3xl font-bold leading-tight text-[#1A1A1A]">
            Create your account
            <br />
            and join <span className="text-[#7A8B3C]">Tandem</span>
          </h1>

          <p className="mb-8 text-sm leading-relaxed text-[#6B7280]">
            Join thousands of clinicians and patients who trust Tandem for smarter, more connected
            healthcare.
          </p>
      ...
      <div className="flex flex-1 flex-col bg-white px-6 py-10 sm:px-12 lg:px-20">
        ...
        <div className="mx-auto w-full max-w-md">
          <h2 className="mb-1 text-2xl font-bold text-[#1A1A1A]">Create your account</h2>
          <p className="mb-6 text-sm text-slate-500">Let&apos;s get started with a few details</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="firstName" className="mb-1.5 block text-xs font-medium text-slate-700">
                  First name
                </label>
              ...
              <div>
                <label htmlFor="lastName" className="mb-1.5 block text-xs font-medium text-slate-700">
                  Last name
                </label>
                <div className="relative">
                  <User size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                      ...
                      {[0, 1, 2].map((i) => (
                        <span
                          ...
                          className={`h-1 w-4 rounded-full ${
                            strength.ratio > i / 3 ? strength.color : 'bg-slate-200'
                          }`}
                        />
                      ))}
                    </span>
            ...
            <label className="flex items-start gap-2 text-xs text-slate-600">
              <input
                ...
                className="mt-0.5 h-4 w-4 rounded border-slate-300 text-[#7A8B3C] focus:ring-[#7A8B3C]"
              />
              <span>
                I agree to the{' '}
                <a href="/terms" className="font-medium text-[#7A8B3C] hover:underline">
                  Terms of Service
                </a>{' '}
                and{' '}
                <a href="/privacy" className="font-medium text-[#7A8B3C] hover:underline">
                  Privacy Policy
                </a>
              </span>
            </label>
            ...
            <p className="mt-1 text-center text-sm text-slate-500">
              ...
              <a href="/login" className="font-medium text-[#7A8B3C] hover:underline">
                Log in
              </a>
            </p>
