...
export default function ConsultationPage() {
  ...
  const handleSaveSoap = useCallback(async (updated: SoapNote) => {
    const { data } = await axios.patch(
      `${process.env.NEXT_PUBLIC_API_URL}/soap/${slug}`,
      updated
    );
    setLiveSoapNote(toSoapNote(data as RawSoap));
  }, [slug]);

  // ✅ Save edited SOAP to DB + emit to patient via socket.io on the backend
  const handlePublishSoap = useCallback(async () => {
    await axios.patch(
      `${process.env.NEXT_PUBLIC_API_URL}/meetings/${slug}/soap/publish`,
      { soap: liveSoapNote },
